...
from app.ml.feature_mapper import map_liver
...
def _shap_explanation(model: Any, X_scaled: np.ndarray, feature_names: List[str]) -> List[Dict]:
    ...
    arr = np.array(raw)
    if arr.ndim == 3:
        # (n_samples, n_features, n_classes) — RandomForest format
        if arr.shape[-1] == 2:
            sv = arr[0, :, 1]
        else:
            # (n_classes, n_samples, n_features) — older format
            sv = arr[1, 0]
    else:
        sv = arr[0]
    return [
...
def predict(data: Dict[str, Any]) -> Dict:
    model         = get("liver_model")
    scaler        = get("liver_scaler")
    feature_names = get("liver_features")

    mapped = map_liver(data)
